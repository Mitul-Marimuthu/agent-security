...
def save_results(result: SimulationResult, path: str) -> None:
    ...
    The output has four top-level keys:
        "stats"           — everything from analyze(), including per-agent
                           leakage summaries and behavior tallies
        "messages"        — the full message transcript (round, sender, to, content)
        "proposals"       — all formal proposals made (round, id, sender, to, content)
        "leakage_records" — one entry per evaluated agent turn, with per-item
                           leakage levels, the behavior label, and a content
                           snippet (truncated to 200 chars to keep file size sane)

    The path is generated by the caller (main.py) using the scenario id and mode
    as a suffix, so running --mode both produces two separate files rather than
    overwriting the first. Prints a confirmation line to stdout on success.
    """
    stats = analyze(result)
    output = {
        "stats": stats,
